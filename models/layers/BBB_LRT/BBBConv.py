"""
Codes are modified from kumar-shridhar/PyTorch-BayesianCNN:
...
class BBBConv1d(ModuleWrapper):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, dilation=1, bias=True, priors=None, groups=1):
        ...
        self.groups = groups   # 1, 4
        print(f"BBBConv1d_groups: {self.groups}")
        # exit("请手动确认此处是否需要进行分组卷积")
        self.use_bias = bias
    ...
    def forward(self, x, sample=True):
        ...
        if self.training or sample:
            eps = torch.empty(act_mu.size()).normal_(0, 1).to(act_var.device)
